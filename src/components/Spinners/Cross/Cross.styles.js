...
const motion = ({ position, unicolor, colors, color }:MotionProps) => keyframes`
    12.5%{
        opacity: 0.75;
...
export const Wrapper = styled.div`
  ...
  border-radius: 1rem;
  ${({ margin,padding,width,height,background }:WrapperProps):string => `
    margin:${margin};
    padding:${padding};
    width:${width};
    height:${height};
...
export const Circle = styled.div`
  ...
  ${({ position,marginCircle,size,color,colors,colorIndex,border }:CircleProps):string => `
    grid-area: ${moveOn(position + 1)};
    margin: ${marginCircle};
    width: ${size};
    height: ${size};
    background: ${color? color:colors[selectFrom(colors, colorIndex)]};
    border:${border};
  `}
`;
